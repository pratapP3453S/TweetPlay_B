...
// //daily views update
const updateDailyViews = async (videoId) => {
    ...
    const viewEntry = video.viewsPerDay.find(
        (entry) => entry.date.getTime() === today
    );

    if (viewEntry) {
        // If there is an entry for today, increment the count
        viewEntry.count += 1;
    } else {
        // If no entry for today, add a new one
        video.viewsPerDay.push({ date: today, count: 1 });
    }

    // video.views += 1; // Increment total views

    await video.save();
};

// //count 10 days views
// const getViewsForLastTenDays = async (videoId) => {
...
const viewCountUpdate = asyncHandler(async (req, res) => {
    const videoId = req.params.videoId;
    if(!videoId){
        throw new ApiError(400, "video id not found not updated viewcount")
    }
    // try {
        const viewsUpdated = await updateDailyViews(videoId);
        return res.status(200).json( new ApiResponse(200, viewsUpdated, "views count updated successfully"))
...
// const getViewForLastTenDays = asyncHandler(async (req, res) => {
//     const videoId = req.params.videoId;
//     if(!videoId){
//         throw new ApiError(400, "video id not found can't get last ten days views")
//     }
//     // try {
//         const todayViews = await getViewsForLastTenDays(videoId);
//         // res.status(200).json(views);
//     // } catch (error) {
//     //     res.status(500).json({ error: error.message });
//     // }
//     return res.status(200).json( new ApiResponse(200, todayViews, "today's views we got"))
// });

const getVideoStatsForLastTenDays = asyncHandler( async( req, res) => {
    const userId = req.user._id;
    if(!userId || !mongoose.Types.ObjectId.isValid(userId)){
        throw new ApiError(400, "user id is undefined or not valid")
    }
    ...
    if(!videosStats || !videosStats.length===0){
        return res.status(404).json( new ApiResponse(404, {}, "No video found for this user"))
    }
    return res.status(200).json( new ApiResponse(201, videosStats, "Videos stats fetched successfully."))
})
